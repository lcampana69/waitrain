...
class Settings:
    ...
    def from_toml(cls, path: pathlib.Path) -> "Settings":
        ...
        direct_api_key = llm.get("api_key", "").strip()
        api_key = direct_api_key or os.environ.get(api_key_env, "").strip()

        if not api_key and api_key_env.lower().startswith("sk-"):
            raise ValueError(
                "La clave de OpenAI parece haberse puesto en `llm.api_key_env`. "
                "Mueve el valor a `llm.api_key` o a la variable de entorno OPENAI_API_KEY."
            )

        if not api_key:
            hint_env = "OPENAI_API_KEY" if api_key_env.lower().startswith("sk-") else api_key_env
            raise ValueError(
                "OpenAI API key is required. Define `llm.api_key` en el TOML o "
                f"exporta la variable de entorno {hint_env}."
            )
