...
def get_settings(config_path: str | None = None) -> Settings:
    """Load settings while converting configuration issues to HTTP errors."""

    try:
        return load_settings(config_path)
    except Exception as exc:  # pragma: no cover - runtime safety
        logger.exception("Configuration error detected")
        raise _http_error(exc, context="config") from exc
...
@app.get("/schema")
async def schema(settings: Settings = Depends(get_settings)):
    engine = get_engine(settings.dsn)
